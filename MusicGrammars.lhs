...
See the bottom of the file for examples of how
to generate sequences using the PTGG framework.

NOTE: modal support is not complete. Currently 
only major and minor are supported fully.
...
Generating Sequences with PTGGs

The following are examples of how to generate using PTGGs. The
functions tsdGen and rGen take a random seed (Int) and return 
a result that is flattened into a single list.

> tsdGen :: Int -> [(RTerm, MP)]
> tsdGen s = toPairs $ snd $ gen tsdRules (mkStdGen s, [NT (T,MP 4 Major 0)]) !! 4

> rGen :: Int -> [(CType, MP)]
> rGen s = toPairs $ snd $ gen (rRules1 qn False) (mkStdGen s, [NT (I,MP 4 Major 0)]) !! 5

These versions print the value in a more easily readable way. 

> tsdGenShow s = putStrLn $ concat $ intersperse "  " $
>     map showDur $ toPairs $ snd $ gen tsdRules (mkStdGen s, [NT (T,MP 4 Major 0)]) !! 4

> rGenShow s = putStrLn $ concat $ intersperse "  " $ map showDur $ toPairs $ snd $ 
